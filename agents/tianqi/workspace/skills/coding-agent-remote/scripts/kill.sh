#!/bin/bash
#
# Kill a remote coding agent tmux session
#
...
if [[ "$FORCE" != true ]]; then
    read -p "Kill session '$SESSION_NAME' on $HOST? (y/N) " -n 1 -r
    echo ""
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Cancelled."
        exit 0
...
# Kill the session on remote
ssh "$HOST" "tmux kill-session -t $SESSION_NAME"
echo "Session '$SESSION_NAME' killed on $HOST."
